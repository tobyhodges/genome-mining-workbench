...
Choose the annotated file ´agalactiae\_A909\_prokka.gbk´
...
- **`.json` file** To know the input file name, the antiSMASH used
  version and the regions data (id,sequence\_data).
...
Streptococcus\_agalactiae\_A909.prokka.zip file.
...
In the JupyterHub, navigate to the pan\_workshop/results/antismash/ folder, select the file we just created, and press the download button at the top

Once in your local machine, in the directory, Streptococcus\_agalactiae\_A909.prokka,
...
Run antiSMASH web server over *S. agalactiae* A909. First, explore the
...
Try Download and annotate *S. thermophilus* genomes strains LMD-9 (MiBiG), and strain CIRM-BIA 65 reference genome.
With the following information, generate the script to run antiSMASH only on the *S. thermophilus* annotated genomes.
...
4. Then you have to call antiSMASH over your variable `mygenome`. Remember the `$` before your variable
